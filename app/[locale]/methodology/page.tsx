    ...
    ? [
        {
          ...
          items: [
            ...
            "日期、农历、命卦、六神、匹配类型和方向都由确定性规则计算。",
            "传统文化方法不等于科学预测，同一输入应得到同一结果。",
          ],
        },
        {
          id: "xiaoliu-ren",
          ...
          items: [
            ...
            "计算：农历月 + 农历日 + 时辰序号，按 mod 6 落位。",
            "23:00-23:59 视为子时；MVP 仍按本地日期对应的农历日期计算。",
            "闰月沿用同名月份数字，同时保留闰月标记。",
    ...
    : [
        ...
        },
        {
          ...
          items: [
            ...
            "23:00-23:59 is treated as Zi hour; MVP keeps the local-date lunar day mapping.",
    ...
    <main className="mx-auto min-h-screen w-full max-w-6xl px-6 py-10 sm:px-8 lg:px-12">
      ...
      <section className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))] p-8 md:p-10">
        ...
        <h1 className="mt-4 font-serif text-5xl text-balance leading-tight">
          {isZh
            ? "先把规则说清楚，再谈如何阅读结果。"
            : "Explain the rules first, then talk about how to read the result."}
        </h1>
        <p className="mt-6 max-w-4xl text-base leading-8 text-[color:var(--color-muted)]">
          {isZh
            ? "FateMirror 的基础事实全部来自可测试的纯函数计算。AI 只允许做可选文案整理，不能重算或改写事实。"
            : "Every base fact in FateMirror comes from testable deterministic functions. AI is allowed only for optional language polish and cannot recalculate or alter the facts."}
        </p>
